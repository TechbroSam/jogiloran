        ...
        <div className="flex items-center justify-between mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-7xl">
            ...
            <nav className="hidden lg:flex gap-8 2xl:ml-16">
                {categories.map((category) => (
                    <Link key={category._id} href={`/products/${category.slug.current}`} className="text-lg font-semibold text-gray-600 transition duration-100 hover:text-orange-700">
                        {category.name}
                    </Link>
                ))}
            </nav>
            <div className="flex items-center">
                <div className="hidden lg:flex divide-x border-r sm:border-l">
                    ...
                    {session ? (
                        <Link href="/profile" className="h-12 w-12 sm:h-20 sm:w-20 md:h-24 md:w-24 flex justify-center items-center text-gray-500 hover:bg-gray-100 transition duration-100"><User /></Link>
                    ) : (
                        <button onClick={openAuthModal} className="h-12 w-12 sm:h-20 sm:w-20 md:h-24 md:w-24 flex justify-center items-center text-gray-500 hover:bg-gray-100 transition duration-100"><User /></button>
                    )}
                </div>
                <div className="flex divide-x border-r">
                    ...
                    <button onClick={toggleMobileMenu} className="h-12 w-12 sm:h-20 sm:w-20 md:h-24 md:w-24 flex justify-center items-center text-gray-500 hover:bg-gray-100 transition duration-100 lg:hidden">
                        <Menu />
                    </button>
                </div>
            </div>
          ...
          <div className="lg:hidden">
            ...
            <div className="fixed inset-y-0 right-0 w-full max-w-xs bg-white z-30 p-6">
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={toggleMobileMenu} className="p-2 -mr-2"><X /></button>
              </div>
              ...
              <nav className="flex flex-col divide-y divide-gray-100">
                ...
                {/* UPDATED: Added padding for consistent spacing */}
                {session ? (
                  <Link href="/profile" onClick={toggleMobileMenu} className="py-3 text-lg font-semibold text-gray-600 flex items-center gap-2"><User size={20}/> My Profile</Link>
                ) : (
                  <button onClick={() => { openAuthModal(); setMobileMenuOpen(false); }} className="py-3 text-lg font-semibold text-gray-600 flex items-center gap-2 text-left"><User size={20}/> Login / Sign Up</button>
                )}
                <button onClick={() => { setIsSearchOpen(true); setMobileMenuOpen(false); }} className="py-3 text-lg font-semibold text-gray-600 flex items-center gap-2 text-left"><Search size={20}/> Search</button>
              </nav>
            </div>
          </div>
        )}
